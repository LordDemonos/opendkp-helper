/**
 * Popup panel: past win history + SVG price chart for items you are bidding on.
 */
(function (global) {
  'use strict';
  ...
  /**
   * Discover items you are bidding on from the live auctions API (popup-side).
   */
            ...
            function considerBid(bid) {
              ...
              if (!mine || (!Number.isNaN(value) && value > (mine.myHighBid || 0))) {
                ...
                if (iid != null) {
                  ...
                  if (!Number.isNaN(n) && n > 0) mine.itemId = n;
                }
                var aid = auction.AuctionId != null ? parseInt(String(auction.AuctionId), 10) : null;
                if (aid && !Number.isNaN(aid)) mine.auctionId = aid;
                if (auction.EndTimestamp) mine.endTimestamp = String(auction.EndTimestamp);
              }
            }

            bids.forEach(considerBid);
            if (mine) found.push(mine);
          });

          // Active list sometimes omits Bids — fetch detail for unmatched auctions (cap 8)
          var needDetail = [];
          if (global.OpenDkpApi.getAuction) {
            auctions.forEach(function (auction) {
              var aid = auction && auction.AuctionId != null ? parseInt(String(auction.AuctionId), 10) : null;
              if (!aid || Number.isNaN(aid)) return;
              var already = found.some(function (f) {
                return f.auctionId === aid;
              });
              if (already) return;
              var bids = Array.isArray(auction.Bids) ? auction.Bids : [];
              if (bids.length) return; // had bids but none were ours
              needDetail.push(aid);
            });
          }
          needDetail = needDetail.slice(0, 8);

          var detailChain = Promise.resolve();
          needDetail.forEach(function (aid) {
            detailChain = detailChain.then(function () {
              return global.OpenDkpApi.getAuction(cfg, aid)
                .then(function (full) {
                  if (!full || typeof full !== 'object') return;
                  var itemName =
                    (full.Item && full.Item.Name) || full.ItemName || full.Name || '';
                  itemName = ItemPriceHistory.sanitizeLookupItemName
                    ? ItemPriceHistory.sanitizeLookupItemName(itemName)
                    : String(itemName).trim();
                  if (!itemName) return;
                  var detailBids = Array.isArray(full.Bids) ? full.Bids : [];
                  var mine = null;
                  detailBids.forEach(function (bid) {
                    if (!bid) return;
                    var cid = bid.CharacterId != null ? String(bid.CharacterId) : '';
                    var cname = String(bid.CharacterName || bid.Name || '').trim();
                    var isMine =
                    ...
                    if (!mine || (!Number.isNaN(value) && value > (mine.myHighBid || 0))) {
                      ...
                      if (full.EndTimestamp) mine.endTimestamp = String(full.EndTimestamp);
                    }
                  });
                  if (mine) found.push(mine);
                })
  ...
  function populateSwitcher(items, selected) {
    ...
    select.innerHTML = items
      .map(function (item) {
        ...
        var clean =
          ...
          item.itemName;
        var label = clean + (item.characterName ? ' (' + item.characterName + ')' : '');
        return (
          '<option value="' +
          escapeHtml(key) +
          '"' +
          (key === itemKey(selected) ? ' selected' : '') +
          '>' +
          escapeHtml(label) +
          '</option>'
        );
      ...
      return ItemPriceHistory.hasAuthToken().then(function (signedIn) {
        return gatherCandidateItems(clientSlug, signedIn).then(function (gathered) {
          ...
          if (!signedIn) {
            ...
            populateSwitcher(items, items[0]);
            var title = $('itemPriceHistoryTitle');
            if (title) {
              title.textContent =
                (ItemPriceHistory.sanitizeLookupItemName &&
                  ItemPriceHistory.sanitizeLookupItemName(items[0].itemName)) ||
                items[0].itemName ||
                'Item';
            }
            return;
          ...
          populateSwitcher(items, selected);
