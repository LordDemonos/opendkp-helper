...
(function (global) {
  ...
  var selectedKey = '';
  /** Item name currently shown in the history panel (for auto-bid toggle). */
  var currentItemName = '';
  var autobidBusy = false;
  var autobidStorageWired = false;
  ...
  function hideAutobidControls() {
    var row = $('itemPriceHistoryAutobid');
    if (row) row.hidden = true;
  }

  /**
   * Show toggle when Settings has an auto-bid rule matching this item.
   * @param {string} itemName
   */
  function refreshAutobidControls(itemName) {
    var row = $('itemPriceHistoryAutobid');
    var btn = $('itemPriceHistoryAutobidToggle');
    var maxEl = $('itemPriceHistoryAutobidMax');
    if (!row || !btn) return Promise.resolve();

    currentItemName = String(itemName || '').trim();
    if (!currentItemName || !global.AutoBid || !AutoBid.getRuleStateForItem) {
      hideAutobidControls();
      return Promise.resolve();
    }

    return AutoBid.getRuleStateForItem(currentItemName).then(function (state) {
      if (!state || !state.matched) {
        hideAutobidControls();
        return;
      }

      var running = !!state.anyEnabled;
      row.hidden = false;
      if (maxEl) {
        if (state.maxDkp != null) {
          maxEl.textContent = 'Max auto-bid: ' + state.maxDkp + ' DKP';
          maxEl.hidden = false;
        } else {
          maxEl.textContent = '';
          maxEl.hidden = true;
      ...
      btn.disabled = !!autobidBusy;
      btn.textContent = running ? 'Auto-bid: On' : 'Auto-bid: Off';
      btn.classList.toggle('on', running);
      btn.classList.toggle('off', !running);
      btn.setAttribute('aria-pressed', running ? 'true' : 'false');
      btn.title = running
        ? 'Disable auto-bid for this item'
        : 'Enable auto-bid for this item';
    });
  }

  function toggleAutobidEnabled() {
    if (autobidBusy || !currentItemName) return Promise.resolve();
    if (!global.AutoBid || !AutoBid.setRulesEnabledForItem || !AutoBid.getRuleStateForItem) {
      return Promise.resolve();
    }
    ...
    return AutoBid.getRuleStateForItem(currentItemName)
      .then(function (state) {
        var enable = !(state && state.anyEnabled);
        return AutoBid.setRulesEnabledForItem({ itemName: currentItemName, enabled: enable });
      })
      .catch(function (err) {
        setHint(err && err.message ? err.message : 'Could not update auto-bid.', true);
      })
      .then(function () {
        autobidBusy = false;
        return refreshAutobidControls(currentItemName);
      });
  }
  ...
  function renderHistoryForItem(item, clientSlug) {
    ...
    refreshAutobidControls(cleanName);
  ...
  function init() {
    ...
    var toggleBtn = $('itemPriceHistoryAutobidToggle');
    if (toggleBtn && !toggleBtn.dataset.iphWired) {
      toggleBtn.dataset.iphWired = '1';
      toggleBtn.addEventListener('click', function () {
        toggleAutobidEnabled();
      });
    }

    if (!autobidStorageWired) {
      autobidStorageWired = true;
      try {
        var api = getApi();
        if (api.storage && api.storage.onChanged) {
          api.storage.onChanged.addListener(function (changes, area) {
            if (area !== 'sync' || !changes.autoBidRules) return;
            if (currentItemName) refreshAutobidControls(currentItemName);
          });
        }
      } catch (_) {}
    }
