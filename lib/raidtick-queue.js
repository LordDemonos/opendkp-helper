...
(function (global) {
      ...
      return readTickDefs().then(function (defs) {
        ...
        return loadQueueDocument(ctx.raidId).then(function (doc) {
          ...
          for (var i = 0; i < defs.length; i++) {
            var slotEntry = slots[String(i)];
            if (!slotEntry) missing.push(i + 1);
            else if (!slotEntry.names || !slotEntry.names.length) emptyNames.push(i + 1);
          }
          if (missing.length) {
            throw new Error('Queue log files for tick(s): ' + missing.join(', '));
          }
          if (emptyNames.length) {
            throw new Error('Add at least one attendee for tick(s): ' + emptyNames.join(', '));
          }
  ...
  var expandedSlotIndex = null;
  var showAddNameForSlot = null;
  ...
  function namesFromServerTick(tick) {
    if (!tick) return [];
    if (global.RaidTickParse && global.RaidTickParse.extractCharactersFromExistingTick) {
      return global.RaidTickParse.extractCharactersFromExistingTick(tick)
        .map(function (c) {
    ...
    if (Array.isArray(tick.Characters)) {
      return tick.Characters.map(function (c) {
        return c && (c.Name != null ? c.Name : c.name) ? String(c.Name != null ? c.Name : c.name) : '';
      }).filter(Boolean);
    }
  ...
  function buildSlotState(qs, slotData, def, idx, ticks) {
    var serverCount = idx < ticks.length ? countNamesOnTick(ticks[idx]) : 0;
    var serverNames = idx < ticks.length ? namesFromServerTick(ticks[idx]) : [];
    var queued = !!(qs && qs.filled);
    var onServer = serverCount > 0;
    var names = [];
    if (queued && slotData && slotData.names && slotData.names.length) {
      names = slotData.names.slice();
    } else if (onServer) {
      names = serverNames.slice();
    }
    var locked = onServer && serverCount > 0 && !queued;
    return {
      queued: queued,
      onServer: onServer,
      nameCount: queued ? (qs.nameCount || names.length) : serverCount,
      serverNameCount: serverCount,
      names: names,
      locked: locked,
      editable: queued && !locked,
      fileName: (slotData && slotData.fileName) || (qs && qs.fileName) || '',
      tickDescription:
        (slotData && slotData.tickDescription) ||
        (qs && qs.tickDescription) ||
        (def && def.description) ||
        ''
    };
  }

  function mapQueuedSlotStates(queuedStates, defs, queueSlots) {
    queueSlots = queueSlots || {};
    return queuedStates.map(function (qs, idx) {
      var slotData = queueSlots[String(idx)];
      return buildSlotState(qs, slotData, defs[idx], idx, []);
    });
  }

  function getSlotDisplayStates(ctx, defs) {
    return loadQueueDocument(ctx.raidId).then(function (doc) {
      var queueSlots = (doc && doc.slots) || {};
      return getQueuedSlots(ctx.raidId, defs.length).then(function (queuedStates) {
        if (!global.OpenDkpApi || !global.OpenDkpApi.getRaid || ctx.raidId == null) {
          return mapQueuedSlotStates(queuedStates, defs, queueSlots);
        }
        return global.OpenDkpApi.getRaid(ctx.cfg, ctx.raidId)
          .then(function (full) {
            ...
            return defs.map(function (def, idx) {
              ...
              return buildSlotState(qs, queueSlots[String(idx)], def, idx, ticks);
            });
          })
          .catch(function () {
            return mapQueuedSlotStates(queuedStates, defs, queueSlots);
  ...
  function updateQueuedSlotNames(raidId, slotIndex, names) {
    if (raidId == null || raidId === '') {
      return Promise.reject(new Error('Select a current raid first.'));
    }
    return loadQueueDocument(raidId).then(function (doc) {
      var key = String(slotIndex);
      var slot = doc.slots && doc.slots[key];
      if (!slot) {
        if (!names || !names.length) return;
        throw new Error('No queued log for tick ' + (slotIndex + 1) + '.');
      }
      if (!names || !names.length) {
        delete doc.slots[key];
        if (expandedSlotIndex === slotIndex) expandedSlotIndex = null;
        showAddNameForSlot = null;
      } else {
        var deduped = global.RaidTickParse
          ? RaidTickParse.dedupeCharacterNames(names)
          : names.slice();
        slot.names = deduped;
        slot.hash = global.RaidTickParse
          ? RaidTickParse.simpleHash(deduped.join('|'))
  ...
  function removeNameFromQueuedSlot(raidId, slotIndex, name) {
    var target = String(name || '').trim().toLowerCase();
    if (!target) return Promise.reject(new Error('Name is required.'));
    return loadQueueDocument(raidId).then(function (doc) {
      var slot = doc.slots && doc.slots[String(slotIndex)];
      if (!slot || !slot.names || !slot.names.length) {
        throw new Error('No queued names for tick ' + (slotIndex + 1) + '.');
      }
      var next = slot.names.filter(function (n) {
        return String(n || '').trim().toLowerCase() !== target;
      });
      return updateQueuedSlotNames(raidId, slotIndex, next);
    });
  }

  function addNameToQueuedSlot(raidId, slotIndex, rawName) {
    var trimmed = String(rawName || '').trim();
    if (!trimmed) return Promise.reject(new Error('Enter a character name.'));
    return loadQueueDocument(raidId).then(function (doc) {
      var slot = doc.slots && doc.slots[String(slotIndex)];
      if (!slot) {
        throw new Error('No queued log for tick ' + (slotIndex + 1) + '.');
      }
      var existing = slot.names ? slot.names.slice() : [];
      var lower = trimmed.toLowerCase();
      if (
        existing.some(function (n) {
          return String(n || '').trim().toLowerCase() === lower;
        })
      ) {
        throw new Error('That name is already on this tick.');
      }
      existing.push(trimmed);
      return updateQueuedSlotNames(raidId, slotIndex, existing);
    });
  }
  ...
  function setSlotButtonDisplay(btn, slotIndex, state) {
    ...
    btn.classList.remove(
      'api-raidtick-slot-queued',
      'api-raidtick-slot-uploaded',
      'api-raidtick-slot-expanded'
    );

    if (state.expanded) {
      btn.classList.add('api-raidtick-slot-expanded');
    }
    ...
    if (state.queued && count > 0) {
      ...
      btn.setAttribute(
        'aria-label',
        'Tick ' + num + ', ' + count + ' names queued — click to show attendees'
      );
    ...
    if (state.onServer && count > 0) {
      ...
      btn.setAttribute(
        'aria-label',
        'Tick ' + num + ', ' + count + ' names on OpenDKP — click to show attendees'
      );
      btn.setAttribute('aria-expanded', state.expanded ? 'true' : 'false');
      return;
    }
  ...
  function slotHasRoster(st) {
    if (!st) return false;
    var count = st.queued ? st.nameCount || 0 : st.serverNameCount || st.nameCount || 0;
    return count > 0;
  }

  function toggleSlotPanel(slotIndex, st) {
    if (!slotHasRoster(st)) return;
    if (expandedSlotIndex === slotIndex) {
      expandedSlotIndex = null;
      showAddNameForSlot = null;
    } else {
      expandedSlotIndex = slotIndex;
      showAddNameForSlot = null;
    }
  }

  function renderSlotPanel(states, ctx) {
    var panel = document.getElementById('apiRaidtickSlotPanel');
    if (!panel) return;

    if (expandedSlotIndex == null || !states || !states[expandedSlotIndex]) {
      panel.hidden = true;
      panel.textContent = '';
      return;
    }

    var st = states[expandedSlotIndex];
    if (!slotHasRoster(st)) {
      panel.hidden = true;
      panel.textContent = '';
      expandedSlotIndex = null;
    ...
    heading.className = 'api-raidtick-panel-heading';
    heading.textContent =
      (st.tickDescription || 'Tick ' + (expandedSlotIndex + 1)) +
      (st.locked ? ' — uploaded' : ' — queued');
    panel.appendChild(heading);

    if (st.locked) {
      var note = document.createElement('p');
      note.className = 'api-raidtick-panel-lock-note';
      note.textContent = 'Uploaded — edit on OpenDKP';
      panel.appendChild(note);
    }

    var list = document.createElement('ul');
    list.className = 'api-raidtick-panel-list';
    (st.names || []).forEach(function (name) {
      var li = document.createElement('li');
      li.className = 'api-raidtick-panel-row';

      var nameEl = document.createElement('span');
      nameEl.className = 'api-raidtick-panel-name';
      ...
      if (st.editable) {
        ...
        removeBtn.className = 'api-raidtick-panel-remove';
        removeBtn.textContent = '−';
        removeBtn.title = 'Remove ' + name;
        removeBtn.setAttribute('aria-label', 'Remove ' + name);
        removeBtn.addEventListener('click', function () {
          removeNameFromQueuedSlot(ctx.raidId, expandedSlotIndex, name)
            .then(function () {
              renderPopupQueueUi();
            })
            .catch(function (e) {
    ...
    footer.className = 'api-raidtick-panel-footer';

    if (st.editable) {
      if (showAddNameForSlot === expandedSlotIndex) {
        var addRow = document.createElement('div');
        addRow.className = 'api-raidtick-panel-add-row';

        var input = document.createElement('input');
        input.type = 'text';
        input.className = 'api-raidtick-panel-add-input';
        input.placeholder = 'Character name';
        input.autocomplete = 'off';

        var addBtn = document.createElement('button');
        addBtn.type = 'button';
        addBtn.className = 'api-raidtick-panel-add-confirm';
        addBtn.textContent = 'Add';

        function submitAdd() {
          addNameToQueuedSlot(ctx.raidId, expandedSlotIndex, input.value)
            .then(function () {
              showAddNameForSlot = null;
              renderPopupQueueUi();
            })
        ...
        addToggle.className = 'api-raidtick-panel-add-toggle';
        addToggle.textContent = '+ Add name';
        addToggle.addEventListener('click', function () {
          showAddNameForSlot = expandedSlotIndex;
          renderSlotPanel(states, ctx);
        });
        footer.appendChild(addToggle);
      }

      var replaceBtn = document.createElement('button');
      replaceBtn.type = 'button';
      replaceBtn.className = 'api-raidtick-panel-replace';
      replaceBtn.textContent = 'Replace log file';
      replaceBtn.addEventListener('click', function () {
        pickFileForSlot(expandedSlotIndex);
      });
  ...
  function renderPopupQueueUi() {
    ...
    var panelEl = document.getElementById('apiRaidtickSlotPanel');
      ...
      .then(function (uploadEnabled) {
        ...
        return readSessionContext().then(function (ctx) {
          ...
          return readTickDefs().then(function (defs) {
            ...
            if (
              expandedSlotIndex != null &&
              (expandedSlotIndex < 0 || expandedSlotIndex >= defs.length)
            ) {
              expandedSlotIndex = null;
              showAddNameForSlot = null;
            }
            ...
            return getSlotDisplayStates(ctx, defs).then(function (states) {
              ...
              if (expandedSlotIndex != null && !slotHasRoster(states[expandedSlotIndex])) {
                expandedSlotIndex = null;
                showAddNameForSlot = null;
              }

              var allQueued = states.every(function (s) {
                return s.queued && (s.nameCount || 0) > 0;
              });
              ...
              defs.forEach(function (def, idx) {
                ...
                var displayState = Object.assign({}, st, {
                  expanded: expandedSlotIndex === idx
                });
                setSlotButtonDisplay(btn, idx, displayState);
                var count = st.queued ? st.nameCount || 0 : st.serverNameCount || st.nameCount || 0;
                ...
                if (st.queued && count > 0) {
                  ...
                  title += '. Click to show attendees.';
                } else if (st.onServer && count > 0) {
                  title += count + ' names on OpenDKP. Click to show attendees.';
                } else {
  ...
  global.RaidTickQueue = {
    ...
    getSlotDisplayStates: getSlotDisplayStates,
    updateQueuedSlotNames: updateQueuedSlotNames,
    removeNameFromQueuedSlot: removeNameFromQueuedSlot,
    addNameToQueuedSlot: addNameToQueuedSlot,
