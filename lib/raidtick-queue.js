...
(function (global) {
  ...
  var RAIDTICK_UPLOAD_SUNSET = true;
      ...
      return readTickDefs().then(function (defs) {
        ...
        return loadQueueDocument(ctx.raidId).then(function (doc) {
          ...
          return readRosterForSlug(ctx.cfg.clientSlug).then(function (roster) {
            if (!Object.keys(roster.map).length) {
              throw new Error('Load roster in Settings (OpenDKP API) before uploading.');
            }
            var allNames = [];
            for (var j = 0; j < defs.length; j++) {
              allNames = allNames.concat(slots[String(j)].names || []);
            }
            var audit = RaidTickParse.auditRosterMapping(allNames, roster.map);
            if (audit.unmapped.length) {
              var sample = audit.unmapped.slice(0, 6).join(', ');
              var suffix = audit.unmapped.length > 6 ? '…' : '';
              throw new Error(
                audit.unmapped.length + ' name(s) not in roster: ' + sample + suffix
              );
            }
            return global.OpenDkpApi.getCognitoUsername({ clientId: ctx.cfg.cognitoClientId }).then(function (updatedBy) {
              if (!updatedBy) {
                throw new Error('Could not read Cognito username from API token — sign in again.');
              }
              return global.OpenDkpApi.getRaid(ctx.cfg, ctx.raidId).then(function (full) {
                var guildClientId = global.OpenDkpApi.resolveGuildClientId(full, roster.clientId);
                if (!guildClientId) {
                  throw new Error('Guild ClientId is missing; reload roster in Settings.');
                }
                if (Number(full.Attendance) === 0) {
                  throw new Error(
                    'This raid has Attendance = No. Create or select a raid with Attendance = Yes before uploading tick logs.'
                  );
                }
                var namesBySlotIndex = [];
                for (var k = 0; k < defs.length; k++) {
                  namesBySlotIndex[k] = (slots[String(k)] && slots[String(k)].names) || [];
                }
  ...
  function setSlotButtonQueued(btn, slotIndex, filled) {
    if (!btn) return;
    if (filled) {
      btn.classList.add('api-raidtick-slot-queued');
      btn.innerHTML =
        '✅<span class="api-raidtick-slot-num">' + (slotIndex + 1) + '</span>';
      btn.setAttribute('aria-label', 'Tick ' + (slotIndex + 1) + ' queued, click to replace');
    } else {
      btn.classList.remove('api-raidtick-slot-queued');
      btn.innerHTML =
        '📄<span class="api-raidtick-slot-num">' + (slotIndex + 1) + '</span>';
      btn.setAttribute('aria-label', 'Pick log file for tick ' + (slotIndex + 1));
    }
  }
  ...
  function showQueueSummaryHint(states) {
    ...
    var filled = states.filter(function (s) {
      return s.filled;
    }).length;
    if (!filled) return;
    ...
    var msg =
      filled === total
        ...
        : filled + ' of ' + total + ' tick logs queued — finish the rest, then click ⬆️.';
    showQueueHint(msg, false);
  }

  function markSlotQueuedOptimistic(slotIndex) {
    var slotsEl = document.getElementById('apiRaidtickSlotButtons');
    if (!slotsEl || !slotsEl.children[slotIndex]) return;
    setSlotButtonQueued(slotsEl.children[slotIndex], slotIndex, true);
    ...
    return isRaidtickUploadEnabled()
      .then(function (uploadEnabled) {
        ...
        return readSessionContext().then(function (ctx) {
          ...
          slotsEl.textContent = '';
          return getQueuedSlots(ctx.raidId, defs.length).then(function (states) {
            var allFilled = states.every(function (s) {
              return s.filled;
            });
            if (uploadBtn) uploadBtn.disabled = !allFilled;

            defs.forEach(function (def, idx) {
              var st = states[idx] || { filled: false };
              var btn = document.createElement('button');
              btn.type = 'button';
              btn.className = 'icon-btn api-session-btn api-raidtick-slot-btn';
              if (st.filled) btn.classList.add('api-raidtick-slot-queued');
              setSlotButtonQueued(btn, idx, st.filled);
              var title = def.description + ' — click to pick log file';
              if (st.filled) {
                title =
                  def.description +
                  ' — queued: ' +
  ...
  function initPopupUi() {
    ...
    if (uploadBtn) {
      uploadBtn.addEventListener('click', function () {
        ...
        if (!window.confirm('Upload all queued tick logs to the current raid on OpenDKP?')) return;
        uploadBtn.disabled = true;
        uploadBtn.textContent = '⏳';
        applyAllQueued()
          .then(function (result) {
            var labels = (result.applied || [])
              .map(function (a) {
                return a.description || 'tick ' + (a.slotIndex + 1);
              })
              .join(', ');
            var hint = document.getElementById('apiSessionHint');
            if (hint) {
          ...
          .catch(function (e) {
            alert('Upload failed: ' + formatApiError(e));
          })
          .then(function () {
