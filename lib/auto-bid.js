...
(function (global) {
  ...
  /** When a matching auction has this many seconds (or fewer) left, poll faster. */
  var URGENT_REMAINING_SEC = 30;
  /** Poll interval used during the urgent window. */
  var URGENT_POLL_SEC = 2;
  ...
  function coerceArray(body) {
    ...
    if (body && Array.isArray(body.BidResults)) return body.BidResults;
  ...
  /**
   * Record auctions where one of our rule characters already has a bid.
   * @param {object} cfg
   * @param {object[]} auctions
   * @param {object[]} rules
   */
  function updateParticipationFromAuctions(cfg, auctions, rules) {
    if (!global.BidParticipation || !global.BidParticipation.writeParticipation) {
      return Promise.resolve();
    }
    var items = [];
    var seen = {};

    function consider(auction, rule, analysis) {
      if (!analysis || analysis.myRank < 0) return;
      var auctionId = readAuctionId(auction);
      var itemName = readItemName(auction);
      if (!itemName) return;
      var key = auctionId != null ? 'a:' + auctionId : 'n:' + normalizeText(itemName);
      if (seen[key]) return;
      seen[key] = true;
      var row = {
        itemName: itemName,
      ...
      if (auctionId != null) row.auctionId = auctionId;
      var endMs = readAuctionEndMs(auction);
      if (endMs) row.endTimestamp = new Date(endMs).toISOString();
      items.push(row);
    }
    ...
    (rules || []).forEach(function (rule) {
      (auctions || []).forEach(function (auction) {
        chain = chain.then(function () {
          var itemName = readItemName(auction);
          if (!itemNameMatches(itemName, rule.itemPattern)) return;
          var analysis = analyzeBids(coerceBids(auction), rule.characterId, readAuctionQuantity(auction));
          consider(auction, rule, analysis);
        });
      });
    });

    return chain.then(function () {
      return global.BidParticipation.writeParticipation({
        clientSlug: cfg && cfg.clientSlug ? cfg.clientSlug : '',
        items: items,
        replaceSource: 'auto-bid'
      });
    });
  }
   ...
   */
  function readAuctionEndMs(auction) {
    if (!auction || typeof auction !== 'object') return null;
    var raw =
      auction.EndTimestamp != null
  ...
  /**
   * True when any enabled-rule-matching auction ends within URGENT_REMAINING_SEC.
   * @param {object[]} auctions
   * @param {object[]} rules — already filtered to enabled
   * @param {number} [nowMs]
   */
  function hasUrgentMatchingAuction(auctions, rules, nowMs) {
    if (!auctions || !auctions.length || !rules || !rules.length) return false;
    ...
    for (var i = 0; i < auctions.length; i++) {
      ...
      for (var j = 0; j < rules.length; j++) {
        if (itemNameMatches(itemName, rules[j].itemPattern)) {
          matched = true;
          break;
        }
      }
      ...
      var remainingSec = (endMs - now) / 1000;
      if (remainingSec > 0 && remainingSec <= URGENT_REMAINING_SEC) return true;
    }
    return false;
  }
  ...
  function computeNextBidAmount(opts) {
    ...
    // All-in: if a full increment would exceed the cap, bid the remaining
    // amount up to the cap when that still beats the current threshold.
    // e.g. leader 380, increment 50, cap 400 → bid 400 (not skip).
    if (target > cap) {
      if (cap > beatBid && cap > myHigh) return cap;
      return null;
    }
  ...
  function persistAutoBidRules(updated) {
  ...
  function readRuleItemPattern(rule) {
    if (!rule || typeof rule !== 'object') return '';
    return String(rule.itemPattern != null ? rule.itemPattern : rule.item || '').trim();
  }

  /**
   * Matching auto-bid rules for an item name (substring match, same as bidding).
   * @param {string} itemName
   * @returns {Promise<{ matched: number, anyEnabled: boolean, maxDkp: number|null, patterns: string[] }>}
   */
  function getRuleStateForItem(itemName) {
    var name = String(itemName || '').trim();
    if (!name) {
      return Promise.resolve({ matched: 0, anyEnabled: false, maxDkp: null, patterns: [] });
    }
    return storageSyncGet(['autoBidRules']).then(function (sync) {
      var raw = Array.isArray(sync.autoBidRules) ? sync.autoBidRules : [];
      var patterns = [];
      var enabledMaxes = [];
      var allMaxes = [];
      ...
      raw.forEach(function (rule) {
        var pattern = readRuleItemPattern(rule);
        if (!pattern || !itemNameMatches(name, pattern)) return;
        matched += 1;
        patterns.push(pattern);
        var maxDkp = parseInt(String(rule.maxDkp != null ? rule.maxDkp : ''), 10);
        var hasMax = !Number.isNaN(maxDkp) && maxDkp >= 1;
        if (hasMax) allMaxes.push(maxDkp);
        if (rule.enabled !== false) {
          anyEnabled = true;
          if (hasMax) enabledMaxes.push(maxDkp);
        }
      });
      var maxPool = anyEnabled && enabledMaxes.length ? enabledMaxes : allMaxes;
      var maxDkp = maxPool.length ? Math.max.apply(null, maxPool) : null;
      return {
        matched: matched,
  ...
  /**
   * Enable or disable all auto-bid rules matching an item name and persist.
   * Preserves incomplete/draft rule rows in storage (does not re-normalize the list).
   * @param {{ itemName: string, enabled: boolean }} opts
   * @returns {Promise<{ changed: boolean, matched: number, enabled: boolean, rules: object[]|null }>}
   */
  function setRulesEnabledForItem(opts) {
    opts = opts || {};
    var itemName = String(opts.itemName || '').trim();
    var enabled = !!opts.enabled;
    if (!itemName) {
      return Promise.resolve({ changed: false, matched: 0, enabled: enabled, rules: null });
    }

    return storageSyncGet(['autoBidRules']).then(function (sync) {
      var raw = Array.isArray(sync.autoBidRules) ? sync.autoBidRules.slice() : [];
      var matched = 0;
      var changed = false;
      var updated = raw.map(function (rule) {
        if (!rule || typeof rule !== 'object') return rule;
        var pattern = readRuleItemPattern(rule);
        if (!pattern || !itemNameMatches(itemName, pattern)) return rule;
        matched += 1;
        var currentlyEnabled = rule.enabled !== false;
        if (currentlyEnabled === enabled) return rule;
        changed = true;
        return Object.assign({}, rule, { enabled: enabled });
      });

      if (!matched || !changed) {
        return { changed: false, matched: matched, enabled: enabled, rules: null };
      }

      return persistAutoBidRules(updated).then(function () {
        return { changed: true, matched: matched, enabled: enabled, rules: updated };
      });
    });
  }
        ...
        return global.OpenDkpApi.placeBid(ctx.cfg, auctionId, body)
          .then(function () {
            ...
            if (global.BidParticipation && global.BidParticipation.writeParticipation) {
              ...
              var iid = readItemId(fullAuction);
              if (iid != null) part.itemId = iid;
              var endMs = readAuctionEndMs(fullAuction);
              if (endMs) part.endTimestamp = new Date(endMs).toISOString();
              global.BidParticipation.writeParticipation({
                clientSlug: ctx.cfg.clientSlug || '',
                items: [part]
              }).catch(function () {});
            }
            return {
  ...
  function runAutoBidTick(opts) {
    ...
    if (!global.OpenDkpApi || !global.OpenDkpApi.getActiveAuctions) {
      return Promise.resolve({ ok: false, reason: 'api_unavailable', results: [], urgentPoll: false });
    }

    return loadAutoBidSettings(opts).then(function (settings) {
      if (!settings.enabled) {
        return { ok: true, skipped: true, reason: 'disabled', results: [], urgentPoll: false };
      }
      if (!settings.hasToken) {
        return { ok: true, skipped: true, reason: 'not_signed_in', results: [], urgentPoll: false };
      }
      if (!settings.clientSlug) {
        return { ok: true, skipped: true, reason: 'no_guild', results: [], urgentPoll: false };
      }
      var activeRules = settings.rules.filter(function (r) {
        return r.enabled;
      });
      if (!activeRules.length) {
        return { ok: true, skipped: true, reason: 'no_rules', results: [], urgentPoll: false };
      }
      ...
      return global.OpenDkpApi.getActiveAuctions(cfg)
        .then(function (body) {
          ...
          if (!auctions.length) {
            return { ok: true, skipped: true, reason: 'no_active_auctions', results: [], urgentPoll: false };
          }

          var urgentPoll = hasUrgentMatchingAuction(auctions, activeRules);
          ...
          return chain.then(function (results) {
            return updateParticipationFromAuctions(cfg, auctions, activeRules).then(function () {
              return { ok: true, skipped: false, results: results, urgentPoll: urgentPoll };
            });
          });
        })
        .catch(function (err) {
  ...
  global.AutoBid = {
    ...
    URGENT_REMAINING_SEC: URGENT_REMAINING_SEC,
    URGENT_POLL_SEC: URGENT_POLL_SEC,
    ...
    getRuleStateForItem: getRuleStateForItem,
    setRulesEnabledForItem: setRulesEnabledForItem,
    ...
    readAuctionEndMs: readAuctionEndMs,
    hasUrgentMatchingAuction: hasUrgentMatchingAuction,
