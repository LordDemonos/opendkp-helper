...
(function (global) {
  ...
  /**
   * Paginated auction / bid-results history — GET /clients/{client}/auctions?page=N
   * Response shape: { BidResults: [...], CurrentPage, TotalPages }.
   * Site "Include All?" means fetch every page (no extra query param).
   * @param {{ apiHost: string, clientSlug: string }} cfg
   * @param {{ page?: number }} [q]
   */
  ...
  /**
   * All bid-results pages (Include All semantics) — concatenates BidResults.
   * @param {{ apiHost: string, clientSlug: string }} cfg
   * @param {{ maxPages?: number }} [opts]
   */
  function getAllPaginatedAuctions(cfg, opts) {
    opts = opts || {};
    var maxPages = opts.maxPages != null ? opts.maxPages : 50;
    if (maxPages < 1) maxPages = 1;

    function pageLoop(page, acc) {
      if (page > maxPages) {
        return Promise.resolve({ BidResults: acc, truncated: true, pagesFetched: page - 1 });
      }
      return getAllAuctions(cfg, { page: page }).then(function (body) {
        var rows = [];
        if (body && Array.isArray(body.BidResults)) rows = body.BidResults;
        else if (body && Array.isArray(body.Models)) rows = body.Models;
        else if (Array.isArray(body)) rows = body;
        var next = acc.concat(rows);
        var current = body && body.CurrentPage != null ? parseInt(String(body.CurrentPage), 10) : page;
        var total = body && body.TotalPages != null ? parseInt(String(body.TotalPages), 10) : page;
        if (Number.isNaN(current) || current < 1) current = page;
        if (Number.isNaN(total) || total < 1) total = page;
        if (current >= total || !rows.length) {
          return { BidResults: next, truncated: false, pagesFetched: page, TotalPages: total };
        }
        return pageLoop(page + 1, next);
      });
    }
  ...
  /**
   * Item history — GET /clients/{client}/items/{itemId}
   * Fallback when auction BidResults filtering is insufficient.
   * @param {{ apiHost: string, clientSlug: string }} cfg
   * @param {number|string} itemId
   */
  function getItemHistory(cfg, itemId) {
    var id = String(itemId || '').trim();
    if (!id) return Promise.reject(new Error('Item id is required'));
    return ensureFreshToken({ clientId: cfg.cognitoClientId }).then(function () {
      return buildHeaders(cfg).then(function (headers) {
        return clientPath(cfg, ['items', id]).then(function (url) {
          return fetchJson(url, { method: 'GET', headers: headers });
        });
      });
    });
  ...
  global.OpenDkpApi = {
    ...
    getAllPaginatedAuctions: getAllPaginatedAuctions,
