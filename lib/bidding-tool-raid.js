...
(function (global) {
  ...
  function showRefreshBanner(raidName, opts) {
    opts = opts || {};
    refreshBannerRaidName = raidName;
    var msg = opts.exhausted
      ? 'Could not auto-refresh to load raid "' +
        raidName +
        '". Refresh this page manually, then OpenDKP Helper can select it.'
      : 'Refresh this page to load the latest raid list, then OpenDKP Helper can select "' +
        raidName +
        '".';
    showBanner(msg, { kind: 'refresh' });
  }

  function raidListAutoRefreshEnabled() {
    var s = getSettings ? getSettings() : null;
    return !s || s.RAID_LIST_AUTO_REFRESH !== false;
  }

  /**
   * Prefer safeReload when latest raid is missing from the Bidding Tool list.
   * Falls back to the refresh banner when capped, cooling down, or focus-guarded.
   */
  function tryAutoRefreshOrBanner(raidName) {
    if (raidListAutoRefreshEnabled() && global.OpenDkpPageReload) {
      var check = global.OpenDkpPageReload.canReload('raid-missing', { raidName: raidName });
      if (check.ok) {
        showBanner('Refreshing page to load raid "' + raidName + '"…', { kind: 'refresh' });
        ...
        log('Raid list auto-refresh skipped:', check.why, raidName);
        showRefreshBanner(raidName, { exhausted: check.why === 'attempt_cap' });
        return false;
      }
    }
    showRefreshBanner(raidName);
    return false;
  }
  ...
  function runCheck() {
    ...
    return resolveLatestRaidViaBackground().then(function (apiResult) {
      ...
      if (shouldSkipMissingRaidAttempt(dropdown, targetName)) {
        tryAutoRefreshOrBanner(targetName);
        return null;
      }
  ...
  function onStorageChanged(changes, area) {
    ...
    if (
      ...
      changes.opendkpRaidListAutoRefresh ||
  ...
  global.BiddingToolRaid = {
    ...
    diagnose: diagnose,
    isBiddingToolPage: isBiddingToolPage
  };
