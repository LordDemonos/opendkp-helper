...
(function (global) {
  ...
  var DISCONNECT_POLL_MS = 2000;
  var STALE_POLL_MS = 8000;
  var RECONNECT_FALLBACK_MS = 4000;
  var STALE_DISMISS_MS = 2 * 60 * 1000;
  var STALE_RELOAD_COUNTDOWN_SEC = 3;
  var STALE_CONSECUTIVE_NEEDED = 2;
  var TIMER_FROZEN_MS = 15000;
  var API_ERROR_BACKOFF_MS = 45000;
  ...
  function staleUiMode() {
    var s = readSettings();
    var mode = s && s.STALE_UI_MODE != null ? String(s.STALE_UI_MODE) : 'warn-reload';
    ...
    if (btn) {
      ...
      disconnectFallbackTimer = setTimeout(function () {
        ...
        if (findDisconnectRoot()) {
          log('Disconnect modal still present after Reconnect click; reloading');
          reloadForDisconnect({ bypassGlobalCooldown: true, skipRecord: true });
        } else {
  ...
  function runStaleCheck() {
    ...
    requestActiveAuctionsSnapshot().then(function (resp) {
      lastDiagnose = {
