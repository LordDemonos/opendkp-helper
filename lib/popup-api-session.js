...
(function (global) {
  ...
  function updateSessionButtonLabel(btn) {
    ...
    return global.OpenDkpApi.getTokenMeta().then(function (meta) {
      ...
      btn.title = meta.isActive
        ? 'Refresh session, reload raids, and refresh your character list'
        : meta.hasToken
  ...
  function parseAccountCharacters(body) {
    ...
    else if (body && Array.isArray(body.Models)) list = body.Models;
    else if (body && Array.isArray(body.Characters)) list = body.Characters;
    else if (body && Array.isArray(body.characters)) list = body.characters;
    return list
      .map(function (c) {
  ...
  /**
   * Pull linked characters and write autoBidCharactersCache (used by auto-bid + price history).
   * @param {{ apiHost: string, clientSlug: string, cognitoClientId?: string }} cfg
   */
  function refreshAndCacheAccountCharacters(cfg) {
    if (!cfg || !cfg.clientSlug || !global.OpenDkpApi || !global.OpenDkpApi.getAccountCharacters) {
      return Promise.resolve([]);
    }
    return global.OpenDkpApi.getAccountCharacters(cfg).then(function (body) {
      var characters = parseAccountCharacters(body);
      var payload = {};
      payload[AUTO_BID_CHARACTERS_CACHE_KEY] = {
        fetchedAt: Date.now(),
        clientSlug: String(cfg.clientSlug).trim().toLowerCase(),
        characters: characters
      };
      return storageLocalSet(payload).then(function () {
        return characters;
      });
    });
  }
    ...
    function handleSessionAction() {
      ...
      return global.OpenDkpApi.getTokenMeta()
        ...
        .then(function (cfg) {
          ...
          var authPromise = hadToken
            ? refreshSessionToken().catch(function () {
                return signInWithStoredCredentials();
              })
            : signInWithStoredCredentials();
          return authPromise.then(function () {
            return refreshAndCacheAccountCharacters(cfg)
              ...
              .then(function () {
                if (global.PopupItemPriceHistory && global.PopupItemPriceHistory.refresh) {
                  return global.PopupItemPriceHistory.refresh().catch(function () {});
                }
              })
              .then(function () {
