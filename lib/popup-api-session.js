...
(function (global) {
   ...
   * @returns {Promise<{ raids: Array, currentId: number|null, cleared?: boolean, verifyFailed?: boolean }>}
   */
  function resolveStoredCurrentRaid(cfg, raids, storedId, summaryJson, opts) {
    ...
    if (storedId == null || storedId === '' || Number.isNaN(Number(storedId))) {
      return Promise.resolve({ raids: raids, currentId: null, cleared: false });
    }
    ...
    if (!global.OpenDkpApi || !global.OpenDkpApi.getRaid) {
      ...
      return Promise.resolve(withCurrent(fallback, idNum));
    }
    ...
    return global.OpenDkpApi.getRaid(cfg, idNum)
      ...
      .catch(function (err) {
        ...
        return withCurrent(next, idNum, { verifyFailed: true });
      });
  }
  ...
  function populateRaidSelect(selectEl, raids, currentId, allowCreate, summaryJson) {
    selectEl.innerHTML = '';
    if (allowCreate) {
    ...
    if (currentId != null && currentId !== '' && !Number.isNaN(Number(currentId)) && !raidIdInList(raids, currentId)) {
      var summary = parseRaidSummary(summaryJson);
            ...
            return loadCreateRaidSettings().then(function (settings) {
              ...
              return global.OpenDkpApi.putRaid(cfg, body).then(function (created) {
                ...
                return fetchRaidsList(cfg).then(function (raids) {
                  ...
                  populateRaidSelect(select, raids, rid, true, JSON.stringify(summary));
                  showCreateRaidUi(false);
