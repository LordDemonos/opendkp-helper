...
(function (global) {
  ...
  function quotedStartsWithTag(quoted, tag) {
    if (!quoted || !tag) return false;
    return new RegExp('^\\s*' + escapeRegex(tag) + '\\b', 'i').test(quoted);
  }
  ...
  function detectLootLine(line, tag) {
    ...
    return quoted ? quotedStartsWithTag(quoted, tag) : false;
  }
